...
class WorkflowItems extends Component {
    ...
    handlers = {
        ...
        onSearchNode: (e) => {
            ...
            const filteredDescriptors = descriptors.filter(descriptor => descriptor.name.toLowerCase().includes(e.target.value.toLowerCase()));
            this.setState({
                        ...
                        <FlexBox flex="1" style={{ overflowY: 'hidden' }}>
                            {
                                textSearch.length ? (
                                    ...
                                    <Collapse style={{ width: '100%' }} activeKey={activeKey.length ? activeKey : Object.keys(descriptors)} onChange={this.handlers.onChangeActiveKey}>
                                        {
                                            Object.keys(descriptors).map(key => (
                                                <Collapse.Panel style={{ background: NODE_COLORS[key].fill }} key={key} header={collapse ? '' : key} showArrow={!collapse}>
